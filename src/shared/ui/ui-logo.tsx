import clsx from "clsx";

export function UiLogo({ className }: { className?: string }) {
  return (
    <div
      className={clsx(className, "flex items-center gap-1 text-2xl font-bold")}
    >
      <Shield className="w-12 h12" />
      PassPut
    </div>
  );
}

export const Shield = ({ className }: { className?: string }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="32"
    height="32"
    viewBox="0 0 24 24"
    className="rotate-45"
  >
    <path
      fill="currentColor"
      d="M7 14q-.825 0-1.412-.587T5 12q0-.825.588-1.412T7 10q.825 0 1.413.588T9 12q0 .825-.587 1.413T7 14m0 4q-2.5 0-4.25-1.75T1 12q0-2.5 1.75-4.25T7 6q1.675 0 3.038.825T12.2 9H21l3 3l-4.5 4.5l-2-1.5l-2 1.5l-2.125-1.5H12.2q-.8 1.35-2.162 2.175T7 18m0-2q1.4 0 2.463-.85T10.875 13H14l1.45 1.025L17.5 12.5l1.775 1.375L21.15 12l-1-1h-9.275q-.35-1.3-1.412-2.15T7 8Q5.35 8 4.175 9.175T3 12q0 1.65 1.175 2.825T7 16"
    />
  </svg>
);
